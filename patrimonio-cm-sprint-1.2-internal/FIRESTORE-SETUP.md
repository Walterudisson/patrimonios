...
A versão `v1.12.0` adiciona somente a fundação PWA. Não é necessário republicar `firestore.rules` ou `storage.rules` quando as versões anteriores já estiverem ativas.
