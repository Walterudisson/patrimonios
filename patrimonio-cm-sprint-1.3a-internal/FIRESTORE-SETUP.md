...
> **Sprint 1.3A / v1.13.0:** mantenha as regras do Firestore já publicadas na v1.12.2 e as regras atuais de Storage. A seleção de divisão é somente um filtro de visualização e não exige mudanças de permissão ou outra configuração obrigatória no Console. As instruções abaixo descrevem a configuração da base anterior, se ainda não tiver sido aplicada.
