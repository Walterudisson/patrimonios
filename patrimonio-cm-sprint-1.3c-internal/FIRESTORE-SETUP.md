...
> **Sprint 1.3C / v1.13.3:** mantenha as regras do Firestore já publicadas na v1.12.2 e as regras atuais de Storage. A lista de progresso por divisão não altera as permissões. Confirme que os índices compostos da v1.13.2 estão ativos e confira eventuais links de criação indicados pelo Firestore nas contagens de entradas pendentes. As instruções abaixo descrevem a configuração da base anterior, se ainda não tiver sido aplicada.
